...
n = 3;                                  %Number of bits
maxst = max(st);                        
i = 2*maxst/(2^n -1);                   %Interval length
mxst = maxst + i;                       %One interval above max value (to have intervals of [-max,max)
partition = [-maxst:i:maxst];           %Quantization intervals
codebook = [-maxst:i:mxst];             %Value quantizer assigns
[index, quants] = quantiz(st, partition,codebook);
st = transpose(quants);
audiowrite("S086GP01_EDP_INP2s/xmt01.wav",st,Fs);

maxgt = max(gt);                        
i = 2*maxgt/(2^n -1);                   %Interval length
mxgt = maxgt + i;                         %One interval above max value (to have intervals of [-max,max)
partition = [-maxgt:i:maxgt];           %Quantization intervals
codebook = [-maxgt:i:mxgt];               %Value quantizer assigns
[index, quants] = quantiz(gt, partition,codebook);
gt = transpose(quants);
audiowrite("S086GP01_EDP_INP2g/xmt01.wav",gt,Fs);

c=cos(2*pi*fc*t);                       %Carrier signal
xm=st+gt;
audiowrite("S086GP01_EDP_INP2aq/xmt01.wav",xm,Fs);
c1=transpose(c);
...
audiowrite("S086GP01_EDP_OUP202/dtmf01.wav",xr,Fs);
