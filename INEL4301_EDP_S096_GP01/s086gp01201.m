...
n=3;                                    %Number of bits
maxxm = max(xm);                        
i = 2*maxxm/(2^n -1);                   %Interval length
mxxm = maxxm + i;                         %One interval above max value (to have intervals of [-max,max)
partition = [-maxxm:i:maxxm];           %Quantization intervals
codebook = [-maxxm:i:mxxm];               %Value quantizer assigns
[index, quants] = quantiz(xm, partition,codebook);
xm = transpose(quants);
audiowrite("S086GP01_EDP_INP2qa/xmt01.wav",xm,Fs);
%
...
audiowrite("S086GP01_EDP_OUP201/dtmf01.wav",xr,Fs);
